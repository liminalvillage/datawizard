---
title: Agreement 4 Annex 1 — Programme Template
type: annex
annex_ref: A4-1
parent_agreement: Agreement 4 — Mentorship
created: '2026-03-17'
updated: '2026-05-16'
status: template
tags:
  - annex
  - ReGenerativa
  - mentorship
  - programme
---
*Programme Annex template for use with the [[Agreement 4 — Mentorship|Mentorship Agreement]]. One annex per Programme. Complete and sign before each Programme is opened for registration. File as: "A4 — [Programme Name] — [YYYY-MM]".*

---

## Programme Identity

**Programme title:** [e.g. "Regenerative Design Intensive"]

**Mentor / host:** [Name, as named in the Mentorship Agreement]

**Hub / location:** [Hub name and address]

**Format:** ☐ Day workshop  ☐ Weekend retreat  ☐ Multi-day residency  ☐ Online  ☐ Hybrid  ☐ Other: ___

**Dates:** From [DD/MM/YYYY] to [DD/MM/YYYY]

**Total Programme hours:** ___

**Language(s):** ___

---

## Access Structure and Capacity

| Tier | Eligibility | Places available | Price |
|---|---|---|---|
| Free | Persons with the highest Validated Contribution (see criteria below) | ___ | €0 |
| Member discounted | Association members in good standing | ___ | €___ |
| Full price | External participants / general public | ___ | €___ |
| **Total capacity** | | **___** | |

**Criteria for free-access eligibility:**
[e.g. "Top 5% of validated contributions in the last 12 months, as determined by the Contribution Ledger; or as jointly agreed by the Association and the Mentor no later than 30 days before the Programme"]

**Member discount rate:** ___% off full price (equivalent to €___ discount)

**Early bird pricing (if applicable):** €___ until [DD/MM/YYYY]

---

## Financial Model

**Selected model:** ☐ Fixed fee  ☐ Revenue share  ☐ Hybrid (base + variable)  ☐ Cost-cover only

### If Fixed Fee:
Mentor receives: €___ (gross, before withholding if applicable), payable within [e.g. 14] days of Programme completion.

### If Revenue Share:
After deduction of all costs listed below, the Mentor receives ___% of Net Programme Revenue.

### If Hybrid:
Mentor receives a base fee of €___ plus ___% of Net Programme Revenue above a threshold of €___.

### If Cost-Cover:
Mentor's costs (travel €___, accommodation €___, materials €___) are reimbursed. No further compensation unless agreed separately.

---

## Cost Waterfall (Revenue Share / Hybrid models)

*Programme revenue is allocated in the following order before distribution to the Mentor:*

| Cost category | Estimated amount | Notes |
|---|---|---|
| Payment processing fees | €___ / ___% | [e.g. bank charges, Stripe] |
| Fiscal charges / taxes | €___ | [e.g. IVA if applicable] |
| Food and catering | €___ | |
| Accommodation (if applicable) | €___ | |
| Materials and supplies | €___ | |
| Hub operational contribution | €___ | |
| Steward and volunteer coordination | €___ | |
| Communication and marketing | €___ | |
| Administration | €___ | |
| **Network Regeneration Contribution** (min. 5% of Net Revenue — GT-4) | ___% | |
| **Total costs** | **€___** | |
| **Net Programme Revenue available for distribution** | **€___** | |

---

## Minimum Viable Threshold

**Minimum participants required to proceed:** ___ (at which point the Programme breaks even)

**Decision deadline** (cancel or confirm): [DD/MM/YYYY] — [e.g. 21 days before the Programme]

**If threshold not met:** ☐ Cancel with full refund to registered participants  ☐ Renegotiate terms  ☐ Proceed at reduced scope (describe): ___

---

## Gift-Economy Downside Allocation

Free access is a structural gift-economy obligation of the network (Agreement 4, Clause 1.3A), not a risk the Mentor underwrites.

- **Estimated free / gift places this Programme:** ___
- **Is Mentor compensation contingent on reaching the Minimum Viable Threshold?** ☐ No - not contingent  ☐ Yes - contingent, on this basis: ___
- **Shortfall allocation.** If free places are offered and the Programme nonetheless runs below the Minimum Viable Threshold, the shortfall between Programme revenue and the Mentor's agreed compensation is borne by the Association, not the Mentor, funded where necessary from the Network Regeneration Fund contributor-support allocation (GT-4.2). The Mentor never carries a loss created by the network's own gift-economy obligation.
- **Cap (optional):** Association shortfall cover for this Programme capped at €___ (blank = no cap).

---

## Cancellation Terms

| Who cancels | How far in advance | Consequence |
|---|---|---|
| Participant | More than [30] days | Full refund |
| Participant | [14–30] days | 50% refund |
| Participant | Less than [14] days | No refund (transfer to another participant permitted) |
| Association or Mentor | Any time | Full refund to all participants; costs split as agreed |
| Force majeure | — | Full refund; no Party liable for costs beyond direct outlays |

---

## Intellectual Property Specific to This Programme

**Recording / photography during the Programme:**
☐ Permitted with participant consent for Association use  ☐ Not permitted  ☐ Permitted with conditions: ___

**Mentor's materials (slides, handouts, exercises):**
☐ Shared with participants for personal use only
☐ May be reproduced by the Association for future Programmes (with Mentor's approval)
☐ Remain exclusively with the Mentor; handouts provided at Programme only

**Co-created materials (if any):** [describe or state "not applicable"]

---

## Voluntary Donations

Participants may make voluntary donations to the Mentor in addition to the Programme price. These shall be:
- Collected separately from Programme registration fees
- Not counted as Programme revenue for the purpose of cost waterfall calculations
- Documented by the Mentor and reported to the Association

---

## Signatures

**For ReGenerativa APS — ETS:** _________________________  Date: _______

**Mentor:** _________________________  Date: _______

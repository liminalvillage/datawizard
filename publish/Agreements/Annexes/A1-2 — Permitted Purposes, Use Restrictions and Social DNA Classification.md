---
title: 'A1-2 — Permitted Purposes, Use Restrictions and Social DNA Classification'
type: annex
annex_ref: A1-2
parent_agreement: Agreement 1 — Asset Use
created: '2026-03-17'
updated: '2026-03-17'
status: template
tags:
  - annex
  - ReGenerativa
  - asset-use
  - social-dna
---
*Annex 2 to the [Asset Use Agreement (Comodato d'Uso)](../Agreement%201%20%E2%80%94%20Asset%20Use.md). Defines the permitted uses of the Asset and classifies them according to the Social DNA Framework. Forms an integral part of that Agreement.*

---

## Section A — Permitted Uses

The Owner permits the Association to use the Asset for the following purposes:

☐ Residential stays for members, stewards, mentors, volunteers, and participants in Association activities
☐ Educational workshops, courses, and training programmes
☐ Ecological restoration and land care activities
☐ Retreats, residencies, and gatherings of a cultural, spiritual, or community nature
☐ Food growing, permaculture, and agroforestry activities
☐ Artistic and creative residencies
☐ Administrative and coordination work of the Association
☐ Events open to the public (subject to applicable licences and noise restrictions)
☐ Other (specify): _______________________________________________

**Maximum simultaneous occupancy permitted:** ___ persons (overnight) / ___ persons (day use)

**Commercial activities permitted by the Association at the Asset:** [describe the scope — e.g. "ticketed workshops and retreats, as long as proceeds are applied to operating costs and the Association's statutory purposes"]

---

## Section B — Restrictions and Excluded Uses

The following uses are **not permitted** without the Owner's prior written consent:

- Subletting or assigning the right of use to third parties outside the Association's network
- Use as a primary or permanent private residence by any individual
- Any use that generates revenue for private benefit rather than the Association's mission
- Structural modifications, extensions, or installations requiring planning permission
- Commercial activities unrelated to the Association's regenerative mission
- Use as security or collateral for any financial obligation

Additional restrictions specified by the Owner:
[Free text — e.g. "The vineyard is excluded from all Association activities. The barn may be used for storage only."]

---

## Section C — Social DNA Classification

*The Association's Social DNA Framework classifies regenerative impact across six domains. For each domain, indicate whether the permitted uses are expected to generate impact, and describe how.*

**1. Environment and Natural Systems**
Expected activities: [e.g. "Land regeneration, composting, rewilding of hedgerow corridors, beekeeping"]
Impact level: ☐ Primary focus  ☐ Significant  ☐ Incidental  ☐ Not applicable

**2. Health and Well-Being**
Expected activities: [e.g. "Yoga and movement retreats, nature therapy, community meals"]
Impact level: ☐ Primary focus  ☐ Significant  ☐ Incidental  ☐ Not applicable

**3. Education and Innovation**
Expected activities: [e.g. "Permaculture design courses, regenerative agriculture training, skill-sharing"]
Impact level: ☐ Primary focus  ☐ Significant  ☐ Incidental  ☐ Not applicable

**4. Economy and Infrastructure**
Expected activities: [e.g. "Shared tool library, circular materials exchange, local procurement networks"]
Impact level: ☐ Primary focus  ☐ Significant  ☐ Incidental  ☐ Not applicable

**5. Community and Culture**
Expected activities: [e.g. "Seasonal gatherings, storytelling, inter-generational exchange, art residencies"]
Impact level: ☐ Primary focus  ☐ Significant  ☐ Incidental  ☐ Not applicable

**6. Governance and Ethical Values**
Expected activities: [e.g. "Participatory decision-making workshops, commons governance research"]
Impact level: ☐ Primary focus  ☐ Significant  ☐ Incidental  ☐ Not applicable

---

## Section D — Ecological Covenant Targets

*In accordance with the Ecological Covenant (GT-2), the Parties may set specific measurable ecological targets for this Asset. These establish a shared commitment, not a personal financial liability for non-attainment.*

| Target | Baseline (at commencement) | Goal (by end of year __) | Measurement method |
|---|---|---|---|
| [e.g. Soil organic matter] | [e.g. 1.8%] | [e.g. 2.5%] | [e.g. Soil test — annual] |
| [e.g. Species count — pollinators] | [___] | [___] | [___] |
| [e.g. Water consumption] | [___] | [___] | [___] |
| [e.g. Waste to landfill] | [___] | [___] | [___] |

*Targets may be updated annually by mutual written agreement.*

---

## Signatures

This Annex forms an integral part of the Asset Use Agreement executed between the Parties on [Date].

**For ReGenerativa APS — ETS:** _________________________  Date: _______

**Owner:** _________________________  Date: _______
